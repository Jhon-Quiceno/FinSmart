...
| Asigné la credencial correcta pero el bot sigue fallando con "Credential with ID ... does not exist" en ejecuciones reales (aunque en el editor, al probar manualmente, sí funciona) | Quedó un workflow **duplicado** (mismo nombre) de un reimport previo, y/o la instancia activa de n8n sigue sirviendo en memoria la versión vieja para las ejecuciones por webhook | Desactivá el workflow, **borralo por completo** (no solo desactivar), reimportá `telegram-expense-bot.json` de cero, y asigná la credencial una sola vez en el que queda — confirmá que solo hay **un** workflow con ese nombre en la lista antes de activar |
| Al mandar una foto, el bot responde con un error de "base64 inválido" (Gemini: "Failed to base64-decode image data", NVIDIA: "Non-base64 digit found", OpenRouter: "No endpoints found that support image input") aunque la imagen sea válida | Esta versión de n8n usa `filesystem` como modo de binarios por defecto (sin estar seteado en ningún lado), y combinado con el task runner, `binary.<prop>.data` devuelve el marcador interno `"filesystem-v2"` en vez del base64 real dentro de un nodo Code — bug conocido de la comunidad de n8n | Agregar `N8N_DEFAULT_BINARY_DATA_MODE=default` a las variables de entorno del servicio `n8n` en `docker-compose.yml` (fuerza modo memoria) y recrear el contenedor (`docker compose up -d n8n`) |
| Al mandar una foto de un recibo real, el bot registra el gasto pero con una descripción rara (texto legal/tributario del recibo en vez del nombre del comercio) | El modelo de visión que respondió (normalmente el de respaldo, no el primero de la prioridad) confundió texto reglamentario del pie del recibo (Gran Contribuyente, resolución DIAN, NIT) con el nombre del negocio — es un problema de calidad del modelo, no un bug de código | El prompt de `ReceiptExtractionService` ya incluye instrucciones explícitas para ignorar ese texto (ver `docs/sprints/sprint2.md`, sección "Validación end-to-end"); si persiste, revisar el orden de `AiProviderRegistry.DEFAULT_PRIORITY` — el proveedor más confiable debería ir primero |
